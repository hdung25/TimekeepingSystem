...
const version = '20260909-consultation-payroll-v1';
...
(async () => {
    ...
    try {
        ...
        await page.waitForFunction(async () => {
            ...
            const key = (await caches.keys()).find(name => name.includes('v162-consultation-payroll'));
            if (!registration?.active || !key) return false;
            const cache = await caches.open(key);
            return !!(await cache.match('/js/payroll-review.js?v=20260908-payroll-review-v2')) &&
                !!(await cache.match('/js/db-service.js?v=20260908-feedback-repair-v1')) &&
        ...
        evidence.browser = await page.evaluate(async () => ({
            ...
            cachedAssets: (await (await caches.open('tdt-chamcong-v162-consultation-payroll-20260909')).keys()).length
        }));
        ...
        await page.screenshot({ path: path.join(root, 'scratch/production-consultation-payroll.png') });
        fs.writeFileSync(path.join(root, 'scratch/production-consultation-payroll.json'), JSON.stringify(evidence, null, 2));
