...
const version = '20260910-student-count-fix-v1';
...
(async () => {
    ...
    try {
        ...
        await page.waitForFunction(async () => {
            ...
            const key = (await caches.keys()).find(name => name === 'tdt-chamcong-v172-student-count-fix-20260910');
            if (!registration?.active || !key) return false;
            const cache = await caches.open(key);
            return !!(await cache.match('/js/payroll-review.js?v=20260908-payroll-review-v2')) &&
                !!(await cache.match('/js/db-service.js?v=20260910-student-count-fix-v1')) &&
        ...
        evidence.browser = await page.evaluate(async () => ({
            ...
            cachedAssets: (await (await caches.open('tdt-chamcong-v172-student-count-fix-20260910')).keys()).length
        }));
        ...
        await page.screenshot({ path: path.join(root, 'scratch/production-auto-attendance.png') });
        fs.writeFileSync(path.join(root, 'scratch/production-auto-attendance.json'), JSON.stringify(evidence, null, 2));
