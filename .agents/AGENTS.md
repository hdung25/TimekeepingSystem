...
* **Fresh-fix invariant:** If a cached/coarse browser position falls outside every configured campus, clear the in-memory value and run exactly one bounded `watchPosition` cycle with `maximumAge: 0`. Keep listening through temporary unavailable/timeout callbacks and accept only a later point that passes the unchanged campus radius. Always clear the watcher/timer. Never widen the radius or bypass the location gate.
* **Acquisition recovery invariant:** If the browser reports `TIMEOUT` or `POSITION_UNAVAILABLE` after its precise/approximate acquisition attempts, clear the in-memory value and run the same single bounded fresh watcher. Never retry `PERMISSION_DENIED` or `UNSUPPORTED`, because the page cannot resolve those states itself. Preserve `PERMISSION_DENIED` from the approximate fallback instead of remapping it to unavailable.
* **Location observability invariant:** A failed attendance location gate must write a best-effort diagnostic containing only the internal error code, permission state, coarse browser/platform category, secure-context/online flags, app version, staff/auth IDs, and timestamp. Never store coordinates, accuracy, distance, full user-agent text, or campus proximity. Diagnostic failure must never replace or hide the original attendance error.
...
* **First-check-in Rules invariant:** `checkInPersonal()` reads the deterministic today and previous-day attendance documents inside its transaction before today's document exists. Firestore Rules must therefore allow an authenticated employee to `get` a missing canonical path `YYYY-MM-DD_<their mapped userId>`. Keep `list` authorization resource-based, reject malformed/cross-staff IDs, and never replace this with a broad staff `read` grant.
