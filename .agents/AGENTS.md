...
* **Acquisition recovery invariant:** If the browser reports `TIMEOUT` or `POSITION_UNAVAILABLE` after its precise/approximate acquisition attempts, clear the in-memory value and run one final fresh acquisition cycle. Never retry `PERMISSION_DENIED` or `UNSUPPORTED`, because the page cannot resolve those states itself.
